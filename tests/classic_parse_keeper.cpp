...
struct correctness_obs : public rlz::build::observer<Alphabet, const typename Alphabet::Symbol*> {

  using Symbol = typename Alphabet::Symbol;
  using SymbolIt = const Symbol *;

  std::int64_t last_delta;

  size_t can_split(size_t, size_t, size_t len, size_t junk, SymbolIt) override
  {
    return len + junk;
  }

  // Returns true if splitting here allocates an another block.
  bool split_as_block(size_t, size_t, size_t, size_t, SymbolIt) override
  { 
    return false;
  }

  // Splits at given phrase.
  void split(size_t source, size_t target, size_t, size_t, SymbolIt, bool is_block) override
  {
