...
template <typename Alphabet, typename SymbolIt, std::int64_t ptr_low, std::int64_t ptr_high, std::int64_t diff_low, std::int64_t diff_high>
struct correctness_obs : public rlz::build::observer<Alphabet, SymbolIt> {
  ...
  void split(size_t source, size_t target, size_t, size_t, const SymbolIt, bool is_block) override
  {
...
template <typename Alphabet>
using PkBuild = typename ParseKeeper<Alphabet>::template builder<const typename Alphabet::Symbol*>;

template <typename Alphabet>
using CorrectObs = correctness_obs<
  Alphabet,
  const typename Alphabet::Symbol*,
...
public:
  ...
  ParseKeeper<Alphabet> get(std::vector<std::shared_ptr<rlz::build::observer<Alphabet, const typename Alphabet::Symbol*>>> obs, const Symbol *buffer)
  {
    auto builder = std::make_shared<PkBuild<Alphabet>>();
    obs.push_back(builder);

    rlz::build::coordinator<Alphabet, const typename Alphabet::Symbol*> c(obs.begin(), obs.end(), buffer);
  ...
  ParseKeeper<Alphabet> unload_get(std::vector<std::shared_ptr<rlz::build::observer<Alphabet, const typename Alphabet::Symbol*>>> obs, const Symbol *buffer)
  {
...
TYPED_TEST(ParseKeep, BuildNoExtraSplits)
{
  ...
  std::vector<std::shared_ptr<build::observer<Alphabet, const typename Alphabet::Symbol*>>> observers {{
    std::make_shared<CorrectObs<Alphabet>>(),
    std::make_shared<observer<Alphabet,  const typename Alphabet::Symbol*>>(ss)
  }};
...
struct copy_obs : public rlz::build::observer<Alphabet, SymbolIt> {
...
TYPED_TEST(ParseKeep, BuildCopySplits)
{
  ...
  std::vector<std::shared_ptr<build::observer<Alphabet, const typename Alphabet::Symbol*>>> observers {{
    std::make_shared<CorrectObs<Alphabet>>(),
    std::make_shared<copy_obs<Alphabet, const typename Alphabet::Symbol*>>(),
    std::make_shared<observer<Alphabet,  const typename Alphabet::Symbol*>>(ss)
  }};
...
template <typename Alphabet, typename SymbolIt>
struct copy_block_obs : public rlz::build::observer<Alphabet, SymbolIt> {
  ...
  // Returns true if splitting here allocates an another block.
  bool split_as_block(size_t source, size_t, size_t, size_t, const SymbolIt) override
  {
    return source == 15;
  }
...
TYPED_TEST(ParseKeep, BuildCopyBlock)
{
  ...
  std::vector<std::shared_ptr<build::observer<Alphabet, const typename Alphabet::Symbol*>>> observers {{
    std::make_shared<CorrectObs<Alphabet>>(),
    std::make_shared<copy_block_obs<Alphabet, const typename Alphabet::Symbol*>>(),
    std::make_shared<observer<Alphabet,  const typename Alphabet::Symbol*>>(ss)
  }};
