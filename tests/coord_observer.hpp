...
class observer : public rlz::build::observer<Alphabet> {
private:
...
public:
  ...
  bool split_as_block(size_t source, size_t target, size_t len, size_t junk_len, const Symbol *data) override
  {
    return false;
  }

  // Splits at given phrase. Returns true if it must be the begin of a new block.
  void split(size_t source, size_t target, size_t len, size_t junk_len, const Symbol *, bool block) override
  {
    if (block) {
      if (not_first_block) {
        (*ss) << "}";
