...
class sd_vector
{
    ...
    protected:
        ...
        friend class rlz::vectors::algorithms::bitsets::detail::sparse_iterator;

        template <typename SparseBV>
        friend class rlz::vectors::algorithms::bitsets::sparse;
    ...
    public:
        ...
        sd_vector(const t_itr begin,const t_itr end, size_t extend)
        {
            if (! is_sorted(begin,end)) {
                throw std::runtime_error("sd_vector: source list is not sorted.");
            }
            if (begin != end and *std::prev(end) > extend) {
                throw std::runtime_error("sd_vector: extend is less than last element");
            }
