...
template <typename Alphabet, typename PtrSize, typename DiffSize, typename PhraseBVRep, typename SubphraseBVRep>
class parse_keeper<Alphabet, PtrSize, DiffSize, PhraseBVRep, SubphraseBVRep>::builder : public rlz::build::observer<Alphabet> {
private:
...
public:
  ...
  static constexpr std::int64_t diff_high = impl::field_limits<DiffSize::value()>::high();
  static constexpr std::int64_t diff_low  = impl::field_limits<DiffSize::value()>::low();
private:
...
public:
  ...
  builder() 
    : source_len(0U), block_ptr(diff_low - 1), state(FILLING), first_phrase(true)
  {
  ...
  bool split_as_block(size_t source, size_t target, size_t len, size_t, const Symbol *) override
  {
    ...
    auto delta_delta  = delta - block_ptr;
    return first_phrase or ((len > 0) and ((delta_delta < diff_low) or (delta_delta > diff_high)));
  }
  ...
  void split(size_t source, size_t target, size_t len, size_t junk_len, const Symbol *bytes, bool new_block) override
  {
    assert(state == FILLING);
    auto delta        = compute_delta(source, target);
    block_ptr         = new_block ? delta : block_ptr;
    auto delta_delta  = delta - block_ptr;
    ...
    if (new_block) {
      ...
      pk_build.ptrs.push_back(delta);
    } else {
      assert((len == 0) or ((delta_delta >= diff_low) and (delta_delta <= diff_high)));
      pk_build.diffs.push_back(len == 0 ? 0U : delta_delta);
    }
